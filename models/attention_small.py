...
def attention_reader_small(rnn_type,hidden_size,dropout_rate_embd,dropout_rate_d,dropout_rate_q,learning_rate,
                     batch_size,optimizer_name,num_epoches,eval_iter,
    ...
    with tf.variable_scope('dense'): # Prediction Step
        ...
        final_prob = tf.layers.dense(bilinear_output, units=num_labels, activation=tf.nn.softmax, 
                                     kernel_initializer=tf.random_uniform_initializer(minval=-0.01, maxval=0.01)) # [batch, dict#]
    ...
    #train_pred = tf.clip_by_value(final_prob, 1e-7, 1.0 - 1e-7)
    #train_unstack = tf.unstack(train_pred,asix=0)
    
    #pred_indice = [tf.nn.top_k(train_unstack[i],tf.size(answer[i])).indices for i in tf.size(answer)]
    test_pred = tf.cast(tf.argmax(final_prob, axis=1), tf.int32)
    #acc = tf.reduce_sum([1 if i == pred_indice[i] else 0 for i in answer],tf.int32)
    #acc1 = tf.reduce_sum([1 if i == pred_indice[i] else 0 for i in answer],tf.int32)
    ...
    loss_op = tf.reduce_sum(-tf.reduce_sum(y_1hot * tf.log(final_prob), reduction_indices=[1]))
    ...
    #        correct = 0
    #        n_examples = 0
    #        for t_x1, t_mask1, t_x2, t_mask2, t_y in all_test:
    #            correct += sess.run(acc, feed_dict = {d_input:t_x1, q_input:t_x2, answer: t_y, training: False})
    #            n_examples += len(t_x1)
